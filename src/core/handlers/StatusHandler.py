...
class StatusHandler(HandlerBase):
    ...
    def __init__(self):
        super().__init__(r'/file/{name:status}')
    ...
    async def get(self, request):
        ...
        async with aws_session.create_client(
                's3',
                region_name=request.app['aws_region'], endpoint_url=request.app['s3_endpoint_url'] ) as s3_client:
            try:
                response = await s3_client.head_object(Bucket=request.query['bucket'], Key=s3_key_name)
